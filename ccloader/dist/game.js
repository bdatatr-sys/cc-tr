...
export async function loadVersion(config) {
    let changelogText = await files.loadText(paths.jailRelative(paths.join(config.gameAssetsDir, 'data/changelog.json')));
    let { changelog } = JSON.parse(changelogText);
    let latestEntry = changelog[0];
    let version = new semver.SemVer(latestEntry.version);
    let hotfix = 0;
    let changes = [];
    if (latestEntry.changes != null)
        changes.push(...latestEntry.changes);
    if (latestEntry.fixes != null)
        changes.push(...latestEntry.fixes);
...
export async function buildNecessaryDOM(config) {
    document.head.appendChild(utils.html('base', {
        attrs: {
    ...
    document.body.appendChild(utils.html('div', {
        id: 'game',
        children: [
            utils.html('canvas', {
                id: 'canvas',
...
export async function loadMainScript(config, eventReceiver) {
...
function callOnImpactInit(callback) {
...
export function getDelegateActivationFunction() {
    return new Promise((resolve) => {
