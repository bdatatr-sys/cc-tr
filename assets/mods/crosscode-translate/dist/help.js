"use strict";
ig.module('crosscode-translate.fixes.help.level-entry')
    ...
    sc.HelpLevelEntry.inject({
        init(colorId, fontColor) {
            ...
            this.setSize(Math.max(this.color.hook.pos.x + this.color.hook.size.x, this.desc.hook.pos.x + this.desc.hook.size.x), Math.max(this.desc.hook.size.y, this.color.hook.size.y));
        },
    });
});
