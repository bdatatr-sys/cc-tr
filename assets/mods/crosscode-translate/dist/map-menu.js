"use strict";
ig.module('crosscode-translate.fixes.map-menu.teleport-to-prepositions')
    ...
    sc.MapAreaContainer.inject({
        onLandmarkPressed(landmark) {
            ...
            try {
                sc.Dialogs.showYesNoDialog = function (text, ...args) {
                    sc.Dialogs.showYesNoDialog = show;
                    if (landmark.teleportQuestion.length > 0)
                        text = landmark.teleportQuestion;
