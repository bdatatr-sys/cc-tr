"use strict";
ig.module('crosscode-translate.fixes.credits-section')
        ...
        createHeader(text, pos, namesEmpty) {
            ...
            for (let { gui } of addedChildren) {
                gui.onVisibilityChange = (visible) => {
                    if (visible) {
                        gui.doStateTransition('DEFAULT', false, false, null, HEADER_TRANSITION_DELAY / sc.credits.speed);
                    }
                    else {
            ...
            for (let i = 0; i < columns; i++) {
                ...
                columnGui.update = () => {
                    ...
                    while (currentNameIndex < children.length) {
                        ...
                        if (absoluteY >= ig.system.height)
                            break;
                        nameHook.doStateTransition('DEFAULT', false, false, null, NAME_TRANSITION_DELAY / sc.credits.speed);
                        currentNameIndex++;
                    }
                };
            }
