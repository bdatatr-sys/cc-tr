...
function guiMapChildren(gui, callback) {
    ...
    for (let i = 0, len = oldChildHooks.length; i < len; i++) {
        ...
        let newChild = callback(oldChildHook.gui, i);
        ...
        init(...args) {
            ...
            if (this.UI2_LIST_BOX_BTN_TICKER_DISPLAY) {
                ...
                newTextChild.tickerHook.maxWidth =
                    this.button.hook.size.x - ((_a = this.button.buttonType.alignXPadding) !== null && _a !== void 0 ? _a : 0) * 2;
...
function createListButtonPatch(gameFeatureModule, getConstructor) {
...
ig.module('ultimate-localized-ui.fixes.arena-menu')
    .requires('ultimate-localized-ui.fixes.item-lists.menu.gui.arena.arena-misc', 'game.feature.menu.gui.arena.arena-misc', 'ultimate-localized-ui.ticker-display')
    .defines(() => {
        ...
        init(...args) {
            ...
            if (this.UI2_LIST_BOX_BTN_TICKER_DISPLAY && this.button.textChild.hook.pos.x > 0) {
                this.button.textChild.tickerHook.maxWidth -=
                    this.button.textChild.hook.pos.x - ((_a = this.button.buttonType.alignXPadding) !== null && _a !== void 0 ? _a : 0);
            }
        },
    });
...
ig.module('ultimate-localized-ui.fixes.new-game-menu')
    ...
    sc.NewGameOptionButton.inject({
        ...
        updateToggleState() {
            ...
            text += enabled ? 'on' : 'off';
            if (this.set.type === sc.TOGGLE_SET_TYPE.SINGLE)
                text += '-radio';
            if (!this.active)
                text += '-grey';
            text += ']';
...
ig.module('ultimate-localized-ui.fixes.item-lists.trade-gui')
    .requires('game.feature.trade.gui.trade-icon', 'game.feature.trade.gui.equip-toggle-stats', 'game.feature.menu.gui.shop.shop-stats', 'game.feature.menu.gui.trade.trade-misc', 'ultimate-localized-ui.ticker-display')
    .defines(() => {
    ...
    sc.TradeIconGui.inject({
        _createContent() {
            ...
            for (let entry of this.entries) {
                ...
                if (level > 0) {
                    newGui.setDrawCallback((width, height) => sc.MenuHelper.drawLevel(level, width, height, numberGfx, isScalable));
                }
...
ig.module('ultimate-localized-ui.fixes.item-lists.social-menu')
    ...
    sc.SocialInfoBox.inject({
        createEquipEntry(itemID, ...args) {
            ...
            if (lastIndex >= 0) {
                let gui = this.equip.removeChildGuiByIndex(lastIndex);
...
ig.module('ultimate-localized-ui.fixes.item-lists.quest-dialog')
    ...
    sc.QuestDialog.inject({
        setQuestRewards(quest, ...args) {
            ...
            guiMapChildren(this.itemsGui, (gui, i) => {
...
//# sourceMappingURL=item-lists.js.map