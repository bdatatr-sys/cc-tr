"use strict";
ig.module('ultimate-localized-ui.ticker-display')
    .requires('impact.base.system', 'impact.feature.gui.gui', 'game.feature.gui.base.text', 'impact.base.font', 'game.feature.font.font-system')
    .defines(() => {
    function triangleWave(x, a) {
        return Math.abs(Math.abs((x - a) % (2 * a)) - a);
    }
    function textLikeToString(text) {
        if (text == null)
            return '';
    ...
    sc.ui2.TickerDisplayHook = ig.Class.extend({
        ...
        speed: 50,
        delayAtBorders: 1,
        constantTextOffset: { x: 0, y: 0 },
        shadowGfx: new ig.Image('mod://ultimate-localized-ui/media/ticker-display-shadow.png'),
        ...
        _tryRenderTicker(renderer) {
            if (this.maxWidth == null || !this.enable)
                return false;
            ...
            let offsetX = (triangleWave(this.timer * this.speed - scaledDelay / 2, maxOffsetX + scaledDelay) -
                scaledDelay / 2).limit(0, maxOffsetX);
            ...
            if (!sc.ui2.debug.hideTickerShadows) {
                ...
                if (offsetX < maxOffsetX) {
                    renderer
                        .addPattern(PATTERN_SHADOW_RIGHT, clippedSizeX - PATTERN_SHADOW_RIGHT.width, 0, 0, 0, PATTERN_SHADOW_RIGHT.width, clippedSizeY)
                        .setCompositionMode(COMPOSITION_MODE);
                }
            }
    ...
    ig.TextParser.parse = function (text, commands, font, ignoreCommands) {
        if (text instanceof sc.ui2.ParsedTextData) {
            if (!ignoreCommands)
                commands.push(...text.commands);
            return text.parsedText;
        }
        return textParserParse.call(this, text, commands, font, ignoreCommands);
    };
    ig.TextBlock.inject({
        setText(text) {
            ...
            if (text instanceof sc.ui2.ParsedTextData) {
                ...
                this.parsedText = ig.TextParser.parse(textLikeToString(text).trim(), this.commands, this.font);
            }
            if (this.maxWidth) {
                this.parsedText = this.font.wrapText(this.parsedText, this.maxWidth, this.linePadding, this.bestRatio, this.commands);
            }
    ...
    sc.ui2.LongHorizontalTextGui = ig.GuiElementBase.extend({
        ...
        setText(text) {
            ...
            const flushBlock = (blockEnd) => {
                ...
                let textBlock = new ig.TextBlock(this.font, new sc.ui2.ParsedTextData(blockParsedText, blockCommands), textBlockSettings);
                this.textBlocks.push(textBlock);
                this.setSize(this.hook.size.x + textBlock.size.x, Math.max(this.hook.size.y, textBlock.size.y));
                blockStart = blockEnd + 1;
                textWidth = 0;
            };
            const { SPLIT_WIDTH } = this.constructor;
    ...
    sc.ui2.IconTextGui = ig.GuiElementBase.extend({
        ...
        init(text, settings) {
            ...
            this.iconTextBlock = new ig.TextBlock(this.font, new sc.ui2.ParsedTextData(firstIcon, iconCommands), textBlockSettings);
            this.textBlock = new ig.TextBlock(this.font, new sc.ui2.ParsedTextData(parsedText, commands), textBlockSettings);
            this.tickerHook = new sc.ui2.TickerDisplayHook(this.hook, (renderer, x, y) => {
                renderer.addText(this.textBlock, x, y);
            });
        ...
        _updateDimensions() {
            this.setSize(this.textBlock.size.x + this.iconTextBlock.size.x, Math.max(this.textBlock.size.y, this.iconTextBlock.size.y));
            this.setPivot(Math.floor(this.hook.size.x / 2), Math.floor(this.hook.size.y / 2));
...
//# sourceMappingURL=ticker-display.js.map