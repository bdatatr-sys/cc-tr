...
ig.module('ultimate-localized-ui.fixes.options-menu')
    .requires('game.feature.menu.gui.options.options-types', 'game.feature.menu.gui.options.options-misc', 'ultimate-localized-ui.ticker-display')
    .defines(() => {
    sc.OptionRow.inject({
        childFocusTargets: [],
        init(optionName, row, rowGroup, ...args) {
            ...
            if (option.type === 'CHECKBOX')
                option.checkboxRightAlign = true;
    ...
    sc.KeyBinderGui.inject({
        ...
        show(...args) {
            ...
            this.box.setSize(Math.max(150, this.anykeyText.hook.size.x + this.anykeyText.hook.pos.y * 2, this.button.hook.pos.x + maxButtonWidth + 4 + maxButtonWidth + this.back.hook.pos.x), this.box.hook.size.y);
        },
    });
});
