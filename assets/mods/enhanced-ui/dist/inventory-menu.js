...
ig.module('ultimate-localized-ui.fixes.inventory-menu')
    .requires('game.feature.menu.gui.item.item-status-trade', 'game.feature.menu.gui.item.item-status-default', 'ultimate-localized-ui.ticker-display')
    .defines(() => {
    sc.ItemStatusTrade.inject({
        _setTradeInfo(itemID) {
            ...
            for (let { gui } of this.content.hook.children) {
                if (!(gui instanceof sc.ItemStatusTrade.BaseEntryType))
                    continue;
    ...
    function addInventoryStatsLevelText(level) {
    ...
    function drawInventoryStatsHeader(renderer, origGfx, callback) {
        let { addGfx } = renderer;
        try {
            renderer.addGfx = (gfx, posX, posY, srcX, srcY, sizeX, sizeY, flipX, flipY) => {
                if (gfx === origGfx) {
                    if (srcX === 280 && srcY === 472 && sizeX === 126 && sizeY === 35) {
                        ...
                        let lvlSizeY = 5;
                        // prettier-ignore
                        const addSubGfx = (subPosX, subPosY, subSizeX, subSizeY) => {
    ...
    sc.ItemStatusDefaultBar.inject({
        ...
        init(name, type, ...args) {
            ...
            for (let { gui } of this.hook.children) {
                if (!(gui instanceof sc.TextGui && gui.text === name))
                    continue;
                this.nameGui = gui;
                switch (type) {
                    case sc.MENU_BAR_TYPE.EXP:
                        if (name === 'EXP')
                            this.nameGui.setText(ig.lang.get('sc.gui.status-hud.exp'));
                        break;
                    case sc.MENU_BAR_TYPE.HP:
                        if (name === 'HP')
                            this.nameGui.setText(ig.lang.get('sc.gui.status-hud.hp'));
                        break;
                    case sc.MENU_BAR_TYPE.SP:
                        if (name === 'SP')
                            this.nameGui.setText(ig.lang.get('sc.gui.status-hud.sp'));
                        break;
                }
                break;
            }
...
//# sourceMappingURL=inventory-menu.js.map