...
ig.module('ultimate-localized-ui.fixes.combat-hud')
    .requires('game.feature.combat.gui.hp-bar-boss', 'game.feature.combat.model.enemy-type', 'game.feature.gui.hud.combat-hud')
    .defines(() => {
...
ig.module('ultimate-localized-ui.fixes.status-hud')
    .requires('game.feature.gui.hud.param-hud', 'game.feature.gui.hud.hp-hud', 'game.feature.gui.hud.sp-hud')
    .defines(() => {
    sc.ParamHudGui.inject({
        init(...args) {
            ...
            for (let param of [this.hp, this.atk, this.def, this.foc]) {
                ...
                let newParamWidth = Math.max(param.hook.size.x, param._text.hook.pos.x + param._text.hook.size.x + param.hook.size.y - 1);
    ...
    sc.HpHudGui.inject({
        ...
        init(...args) {
            this.parent(...args);
            if (!this.UI2_DRAW_LABEL_AS_TEXT_BLOCK)
                return;
