...
ig.module('ultimate-localized-ui.fixes.social-menu')
    .requires('game.feature.menu.gui.social.social-list', 'game.feature.menu.gui.social.social-misc', 'ultimate-localized-ui.text-gui-utils')
    .defines(() => {
    const STATUS_TEXT_GUI_SETTINGS = {
    ...
    sc.SocialList.inject({
        onCreateListEntries(list, ...args) {
            ...
            if (!sc.SocialEntryButton.prototype.UI2_DRAW_STATUS_AS_TEXT_BLOCK)
                return;
            let allStatusTexts = ig.lang.get('sc.gui.menu.social.statuses');
            ...
            for (let text of Object.values(allStatusTexts)) {
                ...
                newStatusWidth = Math.max(newStatusWidth, dummyStatusTextGui.hook.size.x - 1 + (ninepatch.left + 1) + (ninepatch.right + 1));
            }
            dummyStatusTextGui.setText('');
            let statusHeaderStr = ig.lang.get('sc.gui.menu.social.status');
            let statusHeaderHook = this.bg.hook.children.find(({ gui }) => gui instanceof sc.TextGui && gui.text === statusHeaderStr);
            for (let { gui } of list.contentPane.hook.children) {
                if (!(gui instanceof sc.SocialEntryButton))
                    continue;
                let newButtonWidth = gui.button.hook.size.x - (newStatusWidth - gui.statusTextBg.hook.size.x);
    ...
    sc.SocialEntryButton.inject({
        ...
        init(...args) {
            ...
            this.statusTextBg = new ig.BoxGui(this.status.hook.size.x, this.status.hook.size.y, false, this.statusNinepatch);
        ...
        updateMemberStatus(...args) {
            ...
            this.statusTextGui.setDrawCallback(() => {
                sc.ui2.textRecolorDrawCallback(this.statusTextGui.textBlock, this.statusTextRecolors[state]);
            });
        },
    });
