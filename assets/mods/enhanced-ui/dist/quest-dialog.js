...
ig.module('ultimate-localized-ui.fixes.quest-dialog')
    .requires('game.feature.menu.gui.quests.quest-misc', 'game.feature.menu.gui.quests.quest-details', 'ultimate-localized-ui.ticker-display')
    .defines(() => {
    sc.QuestDialog.inject({
        setQuestRewards(quest, hideRewards, finished, ...args) {
            ...
            for (let hook of [sepHook, taskHook, rewardsIconHook, rewardsLineHook, itemsHook]) {
                let gui2 = hook.gui;
                (_a = gui2.initialPosY) !== null && _a !== void 0 ? _a : (gui2.initialPosY = hook.pos.y);
            ...
            if (quest.rewards.exp)
                this.expGui.hook.pos.y += delta;
            if (quest.rewards.money)
                this.creditGui.hook.pos.y += delta;
            if (quest.rewards.cp)
                this.cpGui.hook.pos.y += delta;
        },
    });
    ...
    sc.QuestInfoBox.inject({
        init(...args) {
            this.parent(...args);
            let locationIconHook = this.locationGui.hook.children.find(({ gui }) => gui instanceof ig.ImageGui && gui.image === this.gfx);
    ...
    sc.QuestDetailsView.inject({
        _setQuest(quest, ...args) {
            ...
            for (let hook of [viewHook1, viewHook2]) {
                let gui2 = hook.gui;
                (_a = gui2.initialSizeY) !== null && _a !== void 0 ? _a : (gui2.initialSizeY = hook.size.y);
    ...
    sc.QuestBaseBox.inject({
        ...
        updateDrawables(renderer) {
            ...
            renderer.addColor(color, this.hook.size.x - QUEST_LEVEL_LABEL_WIDTH - QUEST_LEVEL_LABEL_POS_X, QUEST_LEVEL_LABEL_POS_Y, QUEST_LEVEL_LABEL_WIDTH, QUEST_LEVEL_LABEL_HEIGHT);
        },
    });
...
//# sourceMappingURL=quest-dialog.js.map