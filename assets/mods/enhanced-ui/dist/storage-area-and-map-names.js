...
ig.module('ultimate-localized-ui.fixes.storage-area-and-map-names')
    .requires('impact.feature.database.database', 'game.feature.menu.area-loadable', 'impact.base.system', 'impact.feature.storage.storage')
    .defines(() => {
    ...
    ig.Storage.inject({
        init(...args) {
            ...
            try {
                ...
                for (let slot of this.slots)
                    this._fixAreaAndMapNames(slot);
                if (this.autoSlot != null)
                    this._fixAreaAndMapNames(this.autoSlot);
            }
            catch (err) {
        ...
        _addAreaToLookupTable(areaName, areaData) {
            if (areaName == null)
                return;
            ...
            let areaMaps = new Map();
            for (let floor of areaData.floors) {
                for (let map of floor.maps) {
                    if (map.name == null)
                        continue;
                    if (map.name.langUid == null) {
                        throw new Error(`missing langUid of map ${map.name.langUid} in area ${areaName.en_US}`);
                    }
                    if (areaMaps.has(map.name.langUid)) {
                        throw new Error(`duplicate langUid of map ${map.name.langUid} in area ${areaName.langUid}`);
                    }
        ...
        _fixAreaAndMapLangLabel(obj, areaKey, mapKey) {
            if (!isObject(obj))
                return;
            if (!hasKey(obj, areaKey))
                return;
            let areaLabel = obj[areaKey];
            if (!(isObject(areaLabel) &&
...
ig.module('ultimate-localized-ui.fixes.storage-area-and-map-names.map-model-fix')
    ...
    sc.MapModel.inject({
        ...
        validateCurrentPlayerFloor(...args) {
            if (!this.skipCurrentPlayerFloorValidation)
                this.parent(...args);
        },
    });
    sc.CrossCode.inject({
...
//# sourceMappingURL=storage-area-and-map-names.js.map